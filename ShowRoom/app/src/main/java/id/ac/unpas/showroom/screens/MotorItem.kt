...
@Composable
fun MotorItem(item: Motor, navController: NavHostController, onDelete: (String) -> Unit) {
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier
            ...
            Column(modifier = Modifier.weight(3f)) {
                Text(text = "Kapasitas", fontSize = 14.sp)
                Text(text = item.kapasitas, fontSize = 16.sp, fontWeight = FontWeight.Bold)
            }

            Column(modifier = Modifier.weight(3f)) {
                Text(text = "Tanggal Rilis", fontSize = 14.sp)
                Text(text = item.tanggal_rilis, fontSize = 16.sp, fontWeight = FontWeight.Bold)
            }
        ...
        DropdownMenu(
            ...
            subMenus.forEachIndexed { _, s ->
                DropdownMenuItem(onClick = {
                    ...
                    when (s) {
                        "Edit" -> {
                            navController.navigate("edit-motor/${item.id}")
                        }
                        "Delete" -> {
