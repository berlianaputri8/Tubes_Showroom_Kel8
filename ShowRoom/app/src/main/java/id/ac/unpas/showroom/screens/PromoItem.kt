...
@Composable
fun PromoItem(item: Promo, navController:
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier
            ...
            Column(modifier = Modifier.weight(3f)) {
                ...
                Text(text = item.tanggal_awal, fontSize = 16.sp,
                    fontWeight = FontWeight.Bold)
            }

            Column(modifier = Modifier.weight(3f)) {
                Text(text = "Tanggal Akhir", fontSize = 14.sp)
                Text(text = "${item.tanggal_akhir}", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            }

            Column(modifier = Modifier.weight(3f)) {
                Text(text = "Persentase", fontSize = 14.sp)
                Text(text = item.persentase.toString(), fontSize = 16.sp, fontWeight = FontWeight.Bold)
            }
        ...
        DropdownMenu(
            ...
            subMenus.forEachIndexed { _, s ->
                DropdownMenuItem(onClick = {
                    ...
                    when (s) {
                        "Edit" -> {
                            navController.navigate("edit-promo/${item.id}")
                        }
                        "Delete" -> {
